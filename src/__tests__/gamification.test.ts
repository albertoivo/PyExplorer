...
describe('Gamification Logic: Streak Calculation', () => {
    ...
    it('should not update if user already logged in today', () => {
        const result = calculateStreak(5, today, today);
        expect(result).toEqual({
            streak: 5,
            lastActiveDate: today,
            shouldUpdate: false
    ...
    it('should increment streak if last login was yesterday', () => {
        const result = calculateStreak(5, yesterday, today);
        expect(result).toEqual({
            streak: 6,
            lastActiveDate: today,
            shouldUpdate: true
    ...
    it('should reset streak to 1 if last login was 2 days ago', () => {
        const result = calculateStreak(10, twoDaysAgo, today);
        expect(result).toEqual({
            streak: 1,
            lastActiveDate: today,
            shouldUpdate: true
    ...
    it('should handle streak of 0 correctly', () => {
        const result = calculateStreak(0, yesterday, today);
        expect(result).toEqual({
            streak: 1,
            lastActiveDate: today,
            shouldUpdate: true
